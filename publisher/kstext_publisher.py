...
from tkinter import BOTH, END, LEFT, RIGHT, VERTICAL, W, BooleanVar, PanedWindow, StringVar, Tk, filedialog, messagebox, simpledialog, ttk
from tkinter.scrolledtext import ScrolledText
from urllib.parse import quote


DEFAULT_REPO_DIR = Path(r"E:\DESKTOP\project\KsText_shell")
...
def build_index_from_documents(
    ...
    for doc in documents:
        ...
        payload = pack_payload(doc)
        current_sha256 = sha256_bytes(json_bytes(payload))
        old_sha256 = clean_string(existing.get("sha256"))
        is_new = not existing
        is_changed = is_new or current_sha256 != old_sha256
...
def build_index(repo_dir: Path, owner_repo: str, branch: str, bump_changed_version: bool) -> BuildResult:
    documents, existing_by_id, warnings = load_pack_documents(repo_dir)
    return build_index_from_documents(owner_repo, branch, documents, existing_by_id, warnings, bump_changed_version)
...
def commit_and_push(repo_dir: Path, message: str, push: bool) -> list[str]:
    logs: list[str] = []
    run_git(repo_dir, "add", "index.json", "packs")
    status = git_status(repo_dir)
    if not status:
        logs.append("没有可提交的改动")
        return logs
    logs.append("已暂存 index.json 和 packs/")
    commit_output = run_git(repo_dir, "commit", "-m", message).strip()
    if commit_output:
        logs.append(commit_output)
    if push:
        branch = infer_branch(repo_dir)
        push_output = run_git(repo_dir, "push", "origin", branch).strip()
        logs.append(push_output or f"已推送到 origin/{branch}")
    return logs
...
def build_summary(result: BuildResult) -> str:
    stats = result.stats
    return (
        f"共 {stats.total_packs} 个包 | "
        f"新增 {stats.new_packs} | "
        f"变化 {stats.changed_packs} | "
        f"未变 {stats.unchanged_packs}"
    )


def run_cli(args: argparse.Namespace) -> int:
    repo_dir = ensure_repo_dir(args.repo)
    owner_repo = normalize_owner_repo(args.owner_repo or infer_owner_repo(repo_dir))
    branch = args.branch or infer_branch(repo_dir)
    result = build_index(repo_dir, owner_repo, branch, not args.no_bump_version)
    ...
    if args.write_index or args.commit or args.push:
        index_path = write_index_file(repo_dir, result)
        print(f"已写入: {index_path}")

    if args.commit or args.push:
        logs = commit_and_push(repo_dir, args.message, args.push)
        for line in logs:
            if line:
                print(line)
    return 0
...
class PublisherApp(Tk):
    def __init__(self) -> None:
        ...
        self.repo_var = StringVar(value=str(DEFAULT_REPO_DIR if DEFAULT_REPO_DIR.exists() else Path.cwd()))
    ...
    def _build_widgets(self) -> None:
        ...
        ttk.Label(form, text="仓库目录").grid(row=0, column=0, sticky=W, pady=4)
        ttk.Entry(form, textvariable=self.repo_var).grid(row=0, column=1, sticky="ew", padx=8, pady=4)
        ttk.Button(form, text="选择", command=self.pick_repo).grid(row=0, column=2, pady=4)
        ttk.Button(form, text="自动识别", command=self.autodetect).grid(row=0, column=3, padx=(8, 0), pady=4)
        ...
        ttk.Checkbutton(form, text="变化包自动升级 version", variable=self.bump_var).grid(
            row=3, column=0, columnspan=4, sticky=W, pady=(4, 8)
        )

        action_bar = ttk.Frame(frame)
        action_bar.pack(fill="x", pady=(4, 10))
        ttk.Button(action_bar, text="扫描仓库", command=self.scan_repository).pack(side=LEFT)
        ttk.Button(action_bar, text="新建包", command=self.create_pack).pack(side=LEFT, padx=8)
        ttk.Button(action_bar, text="删除包", command=self.delete_pack).pack(side=LEFT, padx=8)
        ttk.Button(action_bar, text="保存当前包", command=self.save_current_metadata).pack(side=LEFT, padx=8)
        ttk.Button(action_bar, text="保存全部包", command=self.save_all_metadata).pack(side=LEFT, padx=8)
        ttk.Button(action_bar, text="仅扫描预览", command=self.preview).pack(side=LEFT, padx=8)
        ttk.Button(action_bar, text="重建 index.json", command=self.rebuild_index).pack(side=LEFT, padx=8)
        ttk.Button(action_bar, text="提交并推送", command=self.commit_push).pack(side=LEFT, padx=8)
        ttk.Button(action_bar, text="一键发布", command=self.full_publish).pack(side=LEFT, padx=8)
        ...
        self.entries_box = ScrolledText(entries_frame, wrap="word", font=("Consolas", 10), height=18)
        self.entries_box.pack(fill=BOTH, expand=True)
    ...
    def autodetect(self) -> None:
        try:
            repo_dir = ensure_repo_dir(self.repo_var.get())
            self.owner_repo_var.set(infer_owner_repo(repo_dir))
            self.branch_var.set(infer_branch(repo_dir))
            self.summary_var.set(f"已识别: {repo_dir}")
    ...
    def on_tree_select(self, _event=None) -> None:
        ...
        self.load_current_form(int(items[0]))
    ...
    def refresh_tree_row(self, index: int) -> None:
        if not self.pack_tree.exists(str(index)):
            return
        doc = self.pack_documents[index]
    ...
    def scan_repository(self) -> None:
        ...
        try:
            ...
            self.sort_documents()
            self.summary_var.set(f"已扫描 {len(self.pack_documents)} 个包")
            self.rebuild_tree()
    ...
    def create_pack(self) -> None:
        try:
            ...
            if file_stem is None:
                return
            file_stem = clean_string(file_stem)
            if not file_stem:
            ...
            if display_name is None:
                return
            display_name = clean_string(display_name) or file_stem

            doc = new_pack_document(repo_dir, file_stem, display_name)
            save_pack_document(doc)
            self.pack_documents.append(doc)
            self.sort_documents()
            self.summary_var.set(f"已新建: {doc.path.name}")
            self.log(f"已新建 {doc.path.name}")
            self.rebuild_tree()
            for index, item in enumerate(self.pack_documents):
                if item.path == doc.path:
                    self.pack_tree.selection_set(str(index))
                    self.pack_tree.focus(str(index))
                    self.load_current_form(index)
                    break
        except Exception as exc:  # pragma: no cover
            messagebox.showerror("KsText Publisher", str(exc))
    ...
    def preview(self) -> None:
        ...
        try:
            ...
            self.persist_form_to_current()
            result = build_index_from_documents(
                owner_repo,
                branch,
                self.pack_documents,
                self.existing_by_id,
            ...
            for pack in result.index_data["packs"][:preview_count]:
                self.log(
                    f"- {pack['name']} | id={pack['id']} | "
                    f"author={pack['author']} | entries={pack['entryCount']} | version={pack['version']}"
                )
    ...
    def commit_push(self) -> None:
        ...
        try:
            ...
            self.persist_form_to_current()
            save_all_pack_documents(self.pack_documents)
            logs = commit_and_push(repo_dir, self.message_var.get().strip() or "update KsText packs", True)
            for line in logs:
                self.log(line)
            self.summary_var.set("提交并推送完成")
            messagebox.showinfo("KsText Publisher", "提交并推送完成")
        except Exception as exc:  # pragma: no cover
            messagebox.showerror("KsText Publisher", str(exc))
    ...
    def full_publish(self) -> None:
        ...
        try:
            ...
            for line in logs:
                self.log(line)
            self.summary_var.set("一键发布完成")
            messagebox.showinfo("KsText Publisher", "一键发布完成")
        except Exception as exc:  # pragma: no cover
            messagebox.showerror("KsText Publisher", str(exc))
